    ...
    >
        <div class="flex w-full max-w-[500px] mx-auto flex items-center justify-around">
            <button 
              ...
              class="w-full h-full"
            >
              <Icon class="mb-1 text-white rounded-full cursor-pointer" name="material-symbols:home-outline" size="35" />
            </button>
            <button 
                @click="userStore.isMenuOverlay = true"
                class="w-full h-full"
            >
              <Icon class="mb-1 text-white rounded-full cursor-pointer" name="material-symbols:edit-square-outline-rounded" size="32" />
            </button>
            <button 
                @click="userStore.isLogoutOverlay = true"
                class="w-full h-full"
            >
                <Icon class="mb-1 text-white rounded-full cursor-pointer" name="ph:sign-out" size="32" />
            </button>
        </div>
    </div>
  </div>
